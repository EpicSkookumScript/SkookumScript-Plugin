...
bool FSkookumScriptRuntimeGenerator::have_project() const
  {
  return FApp::GetProjectName() && FApp::GetProjectName()[0] && FPlatformString::Strcmp(FApp::GetProjectName(), TEXT("None")) != 0;
  }
...
FString FSkookumScriptRuntimeGenerator::make_project_editable()
  ...
  if (!have_project())
    ...
    FString editable_scripts_path = FPaths::ProjectDir() / TEXT("Scripts");
    ...
    if (FPaths::FileExists(editable_project_file_path))
      ...
      FString temp_root_path(FPaths::ProjectDir() / TEXT("Intermediate/SkookumScript"));
      ...
      if (!FPaths::FileExists(temp_project_file_path))
        ...
        if (!IFileManager::Get().Move(*editable_scripts_path, *temp_scripts_path, true, true))
          ...
          FString editable_binary_folder_path = FPaths::ProjectDir() / TEXT("Content/SkookumScript");
          ...
          if (binary_path_name_p)
            ...
            FString config_file_path = FPaths::ProjectConfigDir() / TEXT("DefaultGame.ini");
...
bool FSkookumScriptRuntimeGenerator::can_export_property(FProperty * var_p, int32 include_priority, uint32 referenced_flags)
  {
  ...
  // Only accept classes whose self and super classes are not skipped
  if (FStructProperty * struct_prop_p = CastField<FStructProperty>(var_p))
    {
  ...
  FObjectPropertyBase * object_var_p = CastField<FObjectPropertyBase>(var_p);
  ...
  FStructProperty * struct_var_p = CastField<FStructProperty>(var_p);
  ...
  FArrayProperty * array_var_p = CastField<FArrayProperty>(var_p);
  ...
  FDelegateProperty * delegate_p = CastField<FDelegateProperty>(var_p);
  if (delegate_p)
    {
    signature_function_p = delegate_p->SignatureFunction;
    }
  FMulticastDelegateProperty * multicast_delegate_p = CastField<FMulticastDelegateProperty>(var_p);
  ...
  if (signature_function_p)
    ...
    for (TFieldIterator<FProperty> param_it(signature_function_p); param_it; ++param_it)
      {
...
void FSkookumScriptRuntimeGenerator::initialize_paths()
  ...
  if (have_project())
    {
    project_file_path = get_or_create_project_file(FPaths::ProjectDir(), FApp::GetProjectName(), &m_project_mode);
    }
...
bool FSkookumScriptRuntimeGenerator::initialize_generation_targets()
  ...
  if (have_project())
    {
    GenerationTargetBase::eState project_target_state = m_targets[ClassScope_project].initialize(FPaths::ProjectDir(), FApp::GetProjectName(), &m_targets[ClassScope_engine]);
